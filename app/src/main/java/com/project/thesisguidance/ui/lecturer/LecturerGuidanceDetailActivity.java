...
public class LecturerGuidanceDetailActivity extends AppCompatActivity {
        ...
        db.collection("chat_bimbingan")
                ...
                .addOnFailureListener(new OnFailureListener() {
                    ...
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(LecturerGuidanceDetailActivity.this, "Failed add new comment, try again later", Toast.LENGTH_SHORT).show();
                    }
                });
    }
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            if (task.getResult() != null && task.getResult().size() > 0) {
                                ...
                                Toast.makeText(LecturerGuidanceDetailActivity.this, "Failed get task", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            if (task.getResult() != null && task.getResult().size() > 0) {
                                ...
                                Toast.makeText(LecturerGuidanceDetailActivity.this, "Failed get student", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Log.w(TAG, "Error getting documents.", task.getException());
                        }
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            if (task.getResult() != null && task.getResult().size() > 0) {
                                ...
                                Toast.makeText(LecturerGuidanceDetailActivity.this, "Failed Get Lecturer Data", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Log.w(TAG, "Error getting documents.", task.getException());
                        }
